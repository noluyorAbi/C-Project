...
int createConnection() {
    ...
    struct addrinfo *result;            // Pointer to the list of address results (returned by getaddrinfo)
    ...
    // Translate hostname and port into a list of address structures (IP-addresses)
    ...
    for (rp = result; rp != NULL; rp = rp->ai_next) {
        ...
        if (sock == -1) { continue; } // Socket creation failed, try the next address
